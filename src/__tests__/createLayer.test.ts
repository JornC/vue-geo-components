...
describe("createLayer", () => {
  ...
  it("carries visibility, opacity and zIndex onto the layer", () => {
    const layerProps = { ...base, type: LayerType.EMPTY_VECTOR_LAYER, visibility: false } as LayerProps;
  ...
  it("requests only vector tiles inside the layer's matrix limits", () => {
    const limits = { MinTileRow: 2, MaxTileRow: 4, MinTileCol: 2, MaxTileCol: 4 };
    const layerProps = {
      ...base,
      ...
      matrixLimits: [limits, limits, limits, limits, limits],
    ...
    expect(tileUrlFunction([3, 3, 3], 1, projection)).toBe("https://example.invalid/tiles/3/3/3.pbf");
    expect(tileUrlFunction([3, 9, 3], 1, projection)).toBeUndefined();
    expect(tileUrlFunction([3, 3, 9], 1, projection)).toBeUndefined();
  });
