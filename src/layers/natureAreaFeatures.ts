...
/**
 * Natura 2000 sites as the map holds them: a point per site, and the site's extent alongside it.
 *
 * A site is drawn at its centroid, and picking one flies the map to its bounding box. So a feature
 * here carries a point and four numbers - never the site's actual boundary. Nothing in this module
 * can outline a site; a product that needs outlines fetches the polygons and renders them itself.
 *
 * Fetching and caching stays with the product as well: every AERIUS product reaches a different
 * service for these, and they differ in when the data goes stale.
 */
 ...
 *
 * Both geometries are WKT, which is how AERIUS services publish them. Only the centroid survives as
 * a geometry - the other is reduced to its bounding box on the way in and the shape itself dropped.
 */
 ...
 *
 * Sites whose geometry cannot be read are logged and skipped rather than failing the whole batch -
 * one malformed record should not empty the map.
 */
export function natureAreasToFeatures(areas: NatureArea[]): Feature[] {
  ...
  for (const area of areas) {
    try {
      ...
      // Unreadable geometry: skip this site, keep the rest. Loud, because a site missing from the
      // map is otherwise indistinguishable from one that is not in the data at all.
